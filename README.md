...
此 Fork 新增 Binance、Bybit、OKX、Bitfinex 唯讀資產 Connector，API 憑證加密存於 D1，資產換算 TWD 並納入總資產。支援範圍、設定與自動更新維護方式見 [交易所擴充說明](docs/exchange-connectors.md)。
...
未持有中國信託信用卡時，請在資料來源的中信設定中，將「同步範圍」設為「只同步存款帳戶」並儲存，避免信用卡查詢阻擋存款同步。此選項不會刪除既有信用卡資料。
...
資料來源 → 中國信託 →「中信同步模式」可選「只同步信貸餘額（網銀）」，儲存後使用既有手動／排程同步。此模式只更新分期信貸的剩餘本金，保留但不更新存款與信用卡；不支援循環型信貸、房貸、還款或借款操作。銀行要求額外驗證時會停止，不能保證無人值守登入。貸款餘額以負債扣除淨資產，不包含應繳利息。
