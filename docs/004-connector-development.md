...
Fork 的 Binance／Bybit／OKX／Bitfinex 擴充沿用以下三層 registry、加密設定與 staged persistence；詳見 [交易所擴充說明](exchange-connectors.md)。
...
中國信託設定的公開欄位 `syncCreditCards` 預設省略／true，維持存款與信用卡完整同步。未持有中信信用卡時，可於連接器的「同步範圍」選擇只同步存款，儲存 false；手動與排程皆略過信用卡端點及授權交易對帳。既有信用卡帳戶、帳單與負債不會刪除或歸零；若曾持卡，需注意其資料不再更新。未選擇只同步存款時，信用卡錯誤仍使整次同步失敗，不能將 SVC/9902 一律當成沒有卡片。

## 中信網銀信貸模式

`syncLoansOnly: true` 是獨立網銀信貸模式，優先於 `syncCreditCards`，沿用已加密的登入資料，透過 Browser Rendering 登入官方 `/twrbc/twrbc-invest/qu029/010`。僅讀取可見分期型信貸表格的「貸款餘額」，不操作繳款、申貸或安全設定；額外驗證會停止，不自動重試或註冊裝置。預設不啟用，原有行動銀行 API 流程保持不變。
